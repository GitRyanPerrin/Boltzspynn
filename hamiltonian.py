    ...
    for a1 in range(Nstat):
        ...
        for a2 in range(Nstat):
            ...
            if m1 == m2+1 and s1 == -s2:
                ...
                #H[a1, a2] += 1.0j*alphaR*k*s2 #7
                H[a1, a2] += -betaD/np.sqrt(12)*(1-s1)*m1/ip.R
                
            #if abs(m1-m2)==1 and s1==-s2:
            #if m1 == m2+1 and s1 == -s2:
                #H[a1, a2] += -betaD/np.sqrt(12)*(m1-s1/2)/R

            if abs(m1-m2)==2 and s1 == -s2:
            #if m1-m2==-2 and s1==-s2:
                H[a1, a2] += betaD*k/np.sqrt(6)*s1

            #if m1 == m2+2 and s1 == -s2:
                #H[a1, a2] += betaD/np.sqrt(6)*k*(1-s1)
                #H[a1, a2] += betaD/np.sqrt(6)*k*(1-s1)*(2*m1-1)/2

            if m1-m2==3 and s1 == s2:
                H[a1, a2] += -np.sqrt(3/2)/2*betaD*s1/2*1.0j
                #H[a1, a2] += -np.sqrt(3/2)*1.0j*betaD/2*(m1-3/2)/R*s1
            #if m1-m2==-3 and s1 == s2:
                #H[a1, a2] += -np.sqrt(3/2)*1.0j*betaD/2*(m1+3/2)/R*s1

            if m1 == m2+6 and s1 == s2:
                H[a1, a2] += 0*ip.E0

            # Parallel Magnetic Terms
            if s1 == s2:
                if abs(m1-m2)==3:
                    H[a1, a2] -= np.sqrt(3/2)*s1*betaD*R/lBz**2/2
            ...
            if s1 == -s2:
                if m1 == m2+1:
                    H[a1, a2] += betaD/np.sqrt(12)*R/2/lBz**2
            ...
            if (abs(m1-m2)==2*s1+1 or abs(m1-m2)==2*s1-1) and s1 == -s2:
                H[a1, a2] -= betaD*R/lBx**2/np.sqrt(6)/2*(1-s1)
